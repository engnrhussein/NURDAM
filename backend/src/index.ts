...
app.post('/api/auth/seed-admin', async (c) => {
  ...
  await db
    ...
    .bind('Dr. Ramazan', 'dr.ramazan', passwordHash, 'admin')
    .run();
...
api.post('/users', adminOnly(), async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare(
      ...
      .bind(name.trim(), username, passwordHash, 'user')
      .run();
    ...
    return c.json({
      ...
      user: {
        ...
        name: name.trim(),
        username,
        password, // Return plain password once for admin to share
        role: 'user',
      },
    });
  } catch (err: any) {
...
/**
 * POST /api/v1/logs — Create a session log (timestamp auto-generated)
 */
 ...
 * Merges approved appointments and logs into a single event array.
 */
api.get('/calendar-events', async (c) => {
  ...
  // Map logs to calendar events (point-in-time)
  ...
  const statusColors: Record<string, string> = {
    good: '#10b981', // Emerald
    'needs maintenance': '#f59e0b', // Amber
    offline: '#ef4444', // Red
  };
